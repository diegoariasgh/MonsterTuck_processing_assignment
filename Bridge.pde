...
class Bridge {
  ...
  // Chain constructor
  Bridge(float l, int n, float startx, float starty, float endx, float endy) {
    ...
    for(int i=0; i < numPoints+1; i++) {
      ...
      if (i == 0 ) p = new Particle(startx,starty,4,true);
      else if (i == numPoints) p = new Particle(endx,endy,4,true);
      else p = new Particle(startx + i*len,starty,4,false);
      particles.add(p);
